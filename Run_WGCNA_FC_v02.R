...
plot_preservation_stats(CO_AD_mp_stats, AD_mp_stats, 0, 0.1, 0, 0)


plot_preservation_stats(CO_AD_mp_stats, 0, 0.1)

plot_preservation_stats(AD_mp_stats, 0,0 )

# plot to PDF

setwd(Network_preservation_plots_dir)

#repeat plot until labels are correct (jiggle=T)
...
pdf(paste(Tissue, "module_preservation_stats.PDF", sep="_"), width=13)
plot_preservation_stats(CO_AD_mp_stats, AD_mp_stats, 0, 0.1, 0, 0.1)
dev.off()
...
plotDendroAndColors(CO_geneTree, cbind(CO_modules, CO_AD_modules_new, AD_modules_new), c("CO", "AsymAD", "AD"), dendroLabels=FALSE, hang=0.03, addGuide=TRUE, 
                    guideHang=0.05, main="Frontal Cortex Module Preservation")  
dev.off()
...
labeledHeatmap(Matrix = CO_vs_CO_AD_pTable,
               ...
               colorLabels = TRUE,
               xSymbols = paste("CO_AD ", CO_AD_ModuleLabels, ": ", CO_AD_ModTotals, sep=""),
               ySymbols = paste("CO ", CO_ModuleLabels, ": ", CO_ModTotals, sep=""),
               textMatrix = CO_vs_CO_AD_CountTbl,
               colors = blueWhiteRed(100)[50:100],
               main = paste(Tissue, "correspondence of CO and CO_AD modules", sep=" "),
               cex.text = 1.0, 
               cex.lab = 1.0, 
               setStdMargins = FALSE);
...
pdf(paste(Tissue, "correspondence of CO and CO_AD modules.PDF", sep="_"))
par(mfrow=c(1,1));
par(cex = 1.0);
par(mar=c(9, 10, 5, 2));
# Use function labeledHeatmap to produce the color-coded table with all the trimmings
...
labeledHeatmap(Matrix = CO_vs_CO_AD_pTable,
               ...
               yLabels = paste(" ", CO_ModuleLabels),
               colorLabels = TRUE,
               xSymbols = paste("Pre-MCI ", CO_AD_ModuleLabels, ": ", CO_AD_ModTotals, sep=""),
               ySymbols = paste("CO ", CO_ModuleLabels, ": ", CO_ModTotals, sep=""),
               textMatrix = CO_vs_CO_AD_CountTbl,
               colors = blueWhiteRed(100)[50:100],
               main = "Correspondence of CO and Pre-MCI modules",
               cex.text = 0.8, 
               cex.lab = 0.8, 
               setStdMargins = FALSE);
dev.off()
...
labeledHeatmap(Matrix = CO_vs_CO_AD_pTable,
               ...
               textMatrix = CO_vs_CO_AD_CountTbl,
               colors = blueWhiteRed(100)[50:100],
               main = "Correspondence of CO and AsymAD modules in the FC",
               cex.text = 1, 
               cex.lab = 1, 
               setStdMargins = FALSE);
dev.off()
...
pdf(paste(Tissue, "correspondence of CO_AD and AD modules.PDF", sep="_"))
par(mfrow=c(1,1));
par(cex = 1.0);
par(mar=c(9, 10, 5, 2));
# Use function labeledHeatmap to produce the color-coded table with all the trimmings
...
labeledHeatmap(Matrix = CO_AD_vs_AD_pTable,
               ...
               colorLabels = TRUE,
               xSymbols = paste("AD ", AD_ModuleLabels, ": ", AD_ModTotals, sep=""),
               ySymbols = paste("Pre-MCI ", CO_AD_ModuleLabels, ": ", CO_AD_ModTotals, sep=""),
               textMatrix = CO_AD_vs_AD_CountTbl,
               colors = blueWhiteRed(100)[50:100],
               main = "Correspondence of Pre-MCI and AD modules", 
               cex.text = 0.8, 
               cex.lab = 0.8, 
               setStdMargins = FALSE);
dev.off()
...
labeledHeatmap(Matrix = CO_AD_vs_AD_pTable,
               ...
               textMatrix = CO_AD_vs_AD_CountTbl,
               colors = blueWhiteRed(100)[50:100],
               main = "Correspondence of AsymAD and AD modules in the FC", 
               cex.text = 1, 
               cex.lab = 0.9, 
               setStdMargins = FALSE);
dev.off()
...
pdf(paste(Tissue, "correspondence of CO and AD modules.PDF", sep="_"))
par(mfrow=c(1,1));
par(cex = 1.0);
par(mar=c(9, 10, 5, 2));
# Use function labeledHeatmap to produce the color-coded table with all the trimmings
...
labeledHeatmap(Matrix = CO_vs_AD_pTable,
               ...
               textMatrix = CO_vs_AD_CountTbl,
               colors = blueWhiteRed(100)[50:100],
               main = "Correspondence of CO and AD modules",
               cex.text = 1.0, 
               cex.lab = 0.8, 
               setStdMargins = FALSE);
dev.off()
...
labeledHeatmap(Matrix = CO_vs_AD_pTable,
               ...
               textMatrix = CO_vs_AD_CountTbl,
               colors = blueWhiteRed(100)[50:100],
               main = "Correspondence of CO and AD modules in the FC",
               cex.text = 1.0, 
               cex.lab = 1.0, 
               setStdMargins = FALSE);
dev.off()
...
pdf(paste(Tissue, "correspondence of CO and AD known AD gene modules.PDF", sep="_"))
par(mfrow=c(1,1));
par(cex = 1.0);
par(mar=c(8, 10.4, 2.7, 1)+0.3);
# Use function labeledHeatmap to produce the color-coded table with all the trimmings
...
##### KNOWN AD GENE COLOURS #####

head(CO_module_colour_dataframe)
head(CO_AD_module_colour_dataframe)
head(AD_module_colour_dataframe)

#merge module colours

known_AD_gene_modules<-cbind(CO_module_colour_dataframe, CO_AD_module_colour_dataframe[2], AD_module_colour_dataframe[2])
head(known_AD_gene_modules)

# merge gene symbol and change colnames
colnames(known_AD_gene_modules)<-c("gene_id", "CO", "CO_AD", "AD")
known_AD_gene_modules<-merge(known_AD_gene_modules, knonwn_AD_genes_entrez_ID, by="gene_id")

known_AD_gene_modules<-known_AD_gene_modules[c(1,5,2,3,4)]

dim(known_AD_gene_modules)
head(known_AD_gene_modules)

subset(known_AD_gene_modules, CO=="brown" | CO_AD=="brown" | AD=="brown")

##### CELL TYPES #####
# cell type markers
Cell_type_genes<-c("SYT1",
                   "GFAP",
                   "SLC1A3",
                   "FGFR3",
                   "AQP4",
...
Cell_type_genes_entrez_ID<-subset(gene_symbol_lookup_table2, symbol %in% Cell_type_genes)

Cell_type_genes_entrez_ID

# extract corresponding module colours

head(CO_module_colour_dataframe)
head(CO_AD_module_colour_dataframe)
head(AD_module_colour_dataframe)

Cell_type_genes_changes<-cbind(CO_module_colour_dataframe[CO_module_colour_dataframe$Entrez_ID %in% Cell_type_genes_entrez_ID$gene_id,],
                               CO_AD_module_colour_dataframe[CO_AD_module_colour_dataframe$Entrez_ID %in% Cell_type_genes_entrez_ID$gene_id,][2],
                               AD_module_colour_dataframe[AD_module_colour_dataframe$Entrez_ID %in% Cell_type_genes_entrez_ID$gene_id,][2])

# merge gene symbol and change colnames
colnames(Cell_type_genes_changes)<-c("gene_id", "CO", "CO_AD", "AD")
Cell_type_genes_changes<-merge(Cell_type_genes_changes, Cell_type_genes_entrez_ID, by="gene_id")

Cell_type_genes_changes<-Cell_type_genes_changes[c(1,5,2,3,4)]

Cell_type_genes_changes

# create dir and save results

# directory for correlation plots
dir.create(paste(work_dir,"Cell_types", sep="/"))
Cell_types_dir=paste(work_dir,"Cell_types", sep="/")

setwd(Cell_types_dir)

write.table(Cell_type_genes_changes, file="Cell_type_genes_changes.txt", sep="\t", row.names=F, quote=F)
